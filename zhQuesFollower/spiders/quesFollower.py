        ...
        if response.status != 200:
            ...
            if userCountRet:
                ...
                for sel in res.xpath('//div[contains(@class,"zm-profile-card")]'):
                    item['userDataId'] = sel.xpath('//button/@data-id').extract()
                    # 注意userLinkId中可能有中文
                    item['userLinkId'] = sel.xpath('//a[@class="zm-item-link-avatar"]/@href').re(r'/people/(.*)')
                    item['userImgUrl'] = sel.xpath('//a[@class="zm-item-link-avatar"]/img/@src').extract()
                    item['userName'] = sel.xpath('//h2/a/text()').extract()
                    item['userFollowerCount'] = sel.xpath('//div[@class="details zg-gray"]/a[1]//text()').re(r'(\-?\d+)')
                    item['userAskCount'] = sel.xpath('//div[@class="details zg-gray"]/a[2]//text()').re(r'(\-?\d+)')
                    item['userAnswerCount'] = sel.xpath('//div[@class="details zg-gray"]/a[3]//text()').re(r'(\-?\d+)')
                    item['userUpCount'] = sel.xpath('//div[@class="details zg-gray"]/a[4]//text()').re(r'(\-?\d+)')
